...
const Admin_GetAllOrder = async (req, res) => {
  try {
    ...
    // Find and delete orders completed before 45 days ago
    await AddOrder.deleteMany({ statusIs: 'completed', CompletedDate: { $lt: date45DaysAgo } });
    ...
    if (allOrders.length > 0) {
      ...
      res.json({ message: 'No completed orders found.', success: true });
    }
  }catch (error) {
...
const Admin_AssignTask = async (req, res) => {
...
//  console.log("order", order);
  ...
  if (!kariger) {
    ...
    console.log(error);
    res.status(500).json({ error: 'An error occurred during the process.', message: error.message });
  }
  }
...
module.exports = { Admin_GetAllOrder, Admin_AllOrder, Admin_AssignTask, Admin_DelOrder, Admin_GetOrder }